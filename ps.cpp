...
void calculateMetricsPS(const std::vector<Process *> &sortedProcesses, std::vector<int> &waitingTime, std::vector<int> &turnaroundTime, std::vector<int> &burstTime, double &avgWaitingTime, std::vector<int> &responseTime, std::vector<int> &orderOfExecution)
{
    ...
    for (int i = 0; i < totalProcesses; i++)
    {
        ...
        int priorityOfCurrentProcess = INT_MIN;
        for (int j = 0; j < totalProcesses; j++)
        {
            if (completed[j] == 0)
            {
                if (sortedProcesses[j]->getArrivalTime() > currentTime)
                    break;
                if (sortedProcesses[j]->getPriority() > priorityOfCurrentProcess)
                {
                    priorityOfCurrentProcess = sortedProcesses[j]->getPriority();
                    processToExecute = j;
                }
            }
        }
...
void printMetricsTablePS(const std::vector<Process *> &processes, const std::vector<int> &waitingTime, const std::vector<int> &turnaroundTime, const std::vector<int> &burstTime, double avgWaitingTime, const std::vector<int> &responseTime, const std::vector<int> &orderOfExecution)
{
...
void saveMetricsToFilePS(const std::vector<Process *> &processes, const std::vector<int> &waitingTime, const std::vector<int> &turnaroundTime, const std::vector<int> &burstTime, double avgWaitingTime, const std::vector<int> &responseTime, const std::string &filename, const std::vector<int> &orderOfExecution)
{
...
void schedulePS(const std::vector<Process *> &processes, bool saveToFile, const std::string &filename)
{
    ...
    // Calculate metrics
    calculateMetricsPS(sortedProcesses, waitingTime, turnaroundTime, burstTime, avgWaitingTime, responseTime, orderOfExecution);

    // Print metrics table to console
    std::cout << "EDNOF" << std::endl;
    printMetricsTablePS(sortedProcesses, waitingTime, turnaroundTime, burstTime, avgWaitingTime, responseTime, orderOfExecution);

    // Save metrics to file if requested
    if (saveToFile)
    {
        saveMetricsToFilePS(sortedProcesses, waitingTime, turnaroundTime, burstTime, avgWaitingTime, responseTime, filename, orderOfExecution);
    }
}
